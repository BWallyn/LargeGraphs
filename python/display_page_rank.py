...
def display_page_rank(graph,
	...
	if graph ==1:
		...
		#df = df[df[1]>0.000001] # sinon le plot galère
		plt.scatter(pd.np.log10(df.iloc[:,1].values), pd.np.log10(df.iloc[:,2].values))
		plt.grid()
		plt.xlabel("log(Page Rank)")
		plt.ylabel("log(Out Degree)")
		plt.show()
		#df.rename(columns={0:"index", 1:"rank", 2:"inD", 3:"outD"}, inplace=True)
		...
		df1 = df1[df1["rank_1"]>1e-7] # sinon le plot galère
		...
		df2 = df2[df2["rank_2"]>1e-7] # sinon le plot galère
		df2.drop(columns=["inD_2", "outD_2"], inplace=True)

		merge_df = pd.merge(df1, df2, on=["index"])
		print(merge_df)
		plt.scatter(pd.np.log10(merge_df["rank_1"].values), pd.np.log10(merge_df["rank_2"].values))
		plt.grid()
		plt.xlabel("log(Page Rank) (alpha=0.15)")
		plt.ylabel("log(Page Rank) (alpha=0.2)")
		plt.show()
...
if __name__ == '__main__':
	...
	file_name2="pk_alpha=0.2_alr21-dirLinks.txt"
